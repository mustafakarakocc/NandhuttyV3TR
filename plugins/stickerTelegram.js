...
let handler = async (m, { conn, args, usedPrefix, command }) => {
    if (!args[0]) throw `Şöyle yap:\n${usedPrefix + command} <url>\n\nÖrnek :\n${usedPrefix + command} https://t.me/addstickers/namapack`
    if (!args[0].match(/(https:\/\/t.me\/addstickers\/)/gi)) throw `url salah`
    let packName = args[0].replace('https://t.me/addstickers/', '')
    let res = await fetch(`https://api.telegram.org/bot891038791:AAHWB1dQd-vi0IbH2NjKYUk-hqQ8rQuzPD4/getStickerSet?name=${encodeURIComponent(packName)}`, { method: 'EDİNMEK', headers: { 'User-Agent': 'GoogleBot' } })
    if (!res.ok) throw eror
    let json = await res.json()
    m.reply(`*Total stickers:* ${json.result.stickers.length}
*Estimated completed:* ${json.result.stickers.length * 1.5} detik`.trim())
